...
const features = [
  {
    title: 'シフト作成',
    description: '作業者の休日設定や店舗の営業設定を基に、シフトを自動で生成・編集します。',
    path: '/shift-manager',
    icon: <EditCalendarIcon fontSize="large" color="primary" />
  },
  {
    title: '作業者登録',
    description: 'シフト作成に必要な作業者の情報を登録・編集・削除します。',
    path: '/worker-registration',
    icon: <PersonAddIcon fontSize="large" color="primary" />
  },
  {
    title: '作業者：休日設定',
    description: '作業者ごとの休日や不在日をカレンダーから設定します。',
    path: '/worker-holiday-settings',
    icon: <EventBusyIcon fontSize="large" color="primary" />
  },
  {
    title: '無人販売所：設定',
    description: '店舗全体の休日や棚卸日など、営業に関する設定を行います。',
    path: '/office-holiday-settings',
...
function HomePage() {
  ...
  const { selectedUser } = useUser(); // Get selected user from context
  ...
  const fetchShiftData = async (period) => {
    setLoading(true);
    try {
      // サーバー側のタイムゾーン解釈により初日のデータが欠落する問題への対策として、
      // 検索開始日を意図的に1日前に設定します。
      ...
      if (data.status === 'success') {
        ...
        data.data.forEach(([date, name, task]) => {
          // GASから返される日付文字列をJavaScriptのDateオブジェクトに変換します。
          // タイムゾーンの解釈による日付のずれを防ぐため、カレンダーの日付キーを生成する`toYYYYMMDD`関数をここでも使用し、処理を統一します。
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      const pdf = new jsPDF('l', 'mm', 'a4'); // 横向きに変更
      ...
      <Box sx={{
        ...
        {features.map((feature) => (
          <Card key={feature.title} sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            ...
            <CardActions>
              <Button component={Link} to={feature.path} size="small" variant="contained">
                この機能を使う
              </Button>
            </CardActions>
          </Card>
